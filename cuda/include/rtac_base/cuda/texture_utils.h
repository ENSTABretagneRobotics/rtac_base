...
template <typename T>
void render_texture(const Texture2D<T>& tex, rtac::types::Image<T,DeviceVector>& out)
{
...
template <typename T>
rtac::types::Image<T,DeviceVector> render_texture(const Texture2D<T>& tex)
{
    rtac::types::Image<T,DeviceVector> out;
